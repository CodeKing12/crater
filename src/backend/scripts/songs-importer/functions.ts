...
import ModelDocument from 'rtf-parse/src/rtf/model/Document.js'
...
// Helper functions
const ucwords = (str: string) => str.replace(/\b\w/g, c => c.toUpperCase())
const phpExplode = (separator: string, str: string) => str.split(separator)
...
export function processEwTitle(title: string): string {
...
export async function processEwLyrics(text: string): Promise<string> {
	...
	lines = phpExplode('\n', processed).map(line =>
		processEwLyricsLineSongParts(line)
	)
...
function extractText(nodes: RTFNode[]): string {
	let text = ''
	for (const node of nodes) {
		if (node instanceof Group) {
			// Check for various types of non-content groups
			const isIgnorableGroup = node.children.some(
				(child: RTFNode) => child instanceof RTFText && child.value === '\\*' // \* groups
			)
			const isFontTable = node.children.some(
				(child: RTFNode) => child instanceof Command && child.name === 'fonttbl'
			)
			const isColorTable = node.children.some(
				(child: RTFNode) =>
					child instanceof Command && child.name === 'colortbl'
			)
			const isStyleSheet = node.children.some(
				(child: RTFNode) =>
					child instanceof Command && child.name === 'stylesheet'
			)
			const isFontDefinition = node.children.some(
				(child: RTFNode) =>
					child instanceof Command && child.name.startsWith('fcharset')
			)
...
async function rtfToText(rtfContent: string): Promise<string> {
	...
	const doc: typeof ModelDocument = await rtfParse.parseString(rtfContent)
...
function fallbackRtfConversion(rtfContent: string): string {
	return rtfContent
...
function processEwLyricsLine(line: string): string {
...
function processEwLyricsLineCustom(line: string): string {
	if (line.trim() === '') return '\n'
	let processed = line
...
function processEwLyricsLineSongParts(line: string): string {
	let processed = line
...
// Metadata processing
export function processEwLyricsMetadata(song: Song): string {
	const meta = [
		`Title: ${song.title}`,
...
// Block processing
function processEwBlocks(text: string): string {
	return phpExplode('\n\n', text.trim())
		.map(block => {
			const lines = phpExplode('\n', block)
			const headingMatch = lines[0].match(
				new RegExp(`^(${SONG_SECTION_NAMES.join('|')})`, 'i')
			)
...
export function saveTextFile(song: Song, lyrics: string): void {
	...
	let content = ''
	if (
		CUSTOM_SETTINGS.addMetadataToExportFiles &&
...
export function generateProPresenterGuid(): string {
	return crypto
		.randomBytes(16)
		.toString('hex')
		.toUpperCase()
...
export function saveSongToDatabase(song: Song, lyrics: string): void {
	...
	const insertTransaction = songDB.transaction(() => {
		...
		const songResult: any = songStmt.get({
			title: song.title,
		...
		const lyricRecords = sections.map((section, index) => {
			...
			const isSectionHeader = SONG_SECTION_NAMES.some(name =>
				lines[0].toLowerCase().startsWith(name.toLowerCase())
			)
