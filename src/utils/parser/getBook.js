...
import { nextBook as _nextBook, nextChapter, nextVerse, prevBook as _prevBook, prevChapter, prevVerse, book as _book } from './errors'
...
CV.prototype.next = function() {
  ...
  let cv
  const nextBook = find(osis, {order: this.book.order + 1})

  if (this.getType() === 'book') {
    return (nextBook) ? new CV(nextBook, NEXT.BOOK) : _nextBook
  }
  ...
  if (this.getType() === 'chapter') {
    ...
    if (!cv.success) {
      if (!nextBook) return nextChapter
...
CV.prototype.prev = function() {
  ...
  let cv
  const prevBook = find(osis, {order: this.book.order - 1})

  if (this.getType() === 'book') {
    return (prevBook) ? new CV(prevBook, PREV.BOOK) : _prevBook
  }
  ...
  if (this.getType() === 'chapter') {
    ...
    cv = getChapter(cv, this.chapter - 1)
    if (!cv.success) {
      if (!prevBook) return prevChapter
