...
type InternalChangeFocusFn = (params: { contextName: string, newFocusId?: FocusType }) => void;
...
type EventsSubscription = {
    [event: KeyboardEvent["key"]]: (params: FocusEventHandlerParams & { event: KeyboardEvent }) => void; // the fn is the event handler
}

type ClickEventsSubscription = {
    [clickEvent in ClickEventType]?: (params: FocusEventHandlerParams & { event: MouseEvent, focusId: FocusType }) => void; // the fn is the event handler
}
...
type FocusEventSubscriberFn = (params: EventSubscriberParams) => FocusEventSubscriberFnReturnVal
...
export default function FocusContextProvider(props: ParentProps) {
    ...
    const changeFluidFocus: InternalChangeFocusFn = ({ contextName, newFocusId }) => {
        if (!contextName) return;
    ...
    const mouseDownEventListener = (e: MouseEvent) => {
        ...
        const eventKey = isLeftClick && !isDblClick ? "onClick" : isLeftClick && isDblClick ? "onDblClick" : "onRightClick";
    ...
    onMount(() => {
        ...
        onCleanup(() => document.removeEventListener("mousedown", mouseDownEventListener))
    })

    const subscribeEvent: FocusEventSubscriberFn = ({ name, handlers, defaultCoreFocus, defaultFluidFocus, clickHandlers }) => {
...
export const useFocusContext = () => {
    ...
    if (!value) {
        throw new Error("FocusContext has not been initialized. Check if it is in one of the parent components.");
    }
