...
const InputElement = (props: HTMLStyledProps<"div">) => (
	<styled.div {...props} />
);

export const InputGroup = (_props: InputGroupProps) => {
	const props = mergeProps({ startOffset: "6px", endOffset: "6px" }, _props);
	...
	return (
		...
		>
			...
			<Box
				...
				ps={
					split.startElement
						? `calc(var(--input-height) - ${split.startOffset})`
						: ""
				}
				pe={
					split.endElement
						? `calc(var(--input-height) - ${split.endOffset})`
						: ""
				}
			>
				{split.children}
