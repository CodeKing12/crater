...
const BibleViewer = () => {
  ...
  function updateInput(book: BookInfo, chapter: number, verse: number | string) {
    if (book?.name) {
      setInput(`${getName(book)} ${chapter}:${verse}`)
    }
  }

  const sendVerseToHomePage = (book: string, chapter: string, verse: string) => {
    const verseData = {
      book,
      chapter,
      verse,
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.key === "c") {
        ...
        if (shouldHide) {
          ...
          sendVerseToHomePage(getName(book), chapter.toString(), verse.toString());
        }
        setHidden(shouldHide);
      } else if (e.key === "ArrowDown") {
        // Navigate to the next verse
        const nextVerse = (parseInt(navigatedVerse || highlightVerse || "0") + 1).toString();
        if (parseInt(nextVerse) <= (book?.versesPerChapter?.[chapter - 1] ?? 1)) {
          setNavigatedVerse(nextVerse);
          updateInput(book, chapter, nextVerse);
        }
      } else if (e.key === "ArrowUp") {
        // Navigate to the previous verse
        const prevVerse = (parseInt(navigatedVerse || highlightVerse || "2") - 1).toString();
  ...
  return (
    <div className={`h-full w-full max-w-full max-h-full relative overflow-hidden px-2 controls ${karla.className}`}>
      ...
      <div className="absolute top-20 z-10 w-full flex justify-center">
        <div className="relative flex flex-col gap-1 items-start overflow-auto w-full max-w-4xl h-[calc(100vh-80px)]" ref={verseListRef}>
          {Object.keys(displayChapter).map((verseNum) => (
            <button
              ...
              onClick={() => changeScripture(book, chapter.toString(), verseNum)}
              style={{
                backgroundColor:
